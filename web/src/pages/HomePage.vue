...
async function saveCasePartField(item, part, field, value) {
  ...
  try {
    ...
    expandedCaseParts.value = expandedCaseParts.value.map((value) => value.id === updated.id ? updated : value)
    await loadRepairRegistry()
    showToast(field === 'supplierId' ? 'Поставщик детали сохранён' : 'Дата доставки сохранена')
  } catch (error) {
    showToast(error.message)
  }
}
