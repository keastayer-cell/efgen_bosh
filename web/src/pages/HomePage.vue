  ...
  <template v-else>
    ...
    <main class="page-shell">
      ...
      <section v-if="activeSection === 'cases'" class="workspace cases-workspace">
        <div class="toolbar"><label class="search-field"><span>⌕</span><input v-model="caseSearch" type="search" placeholder="Поиск по VIN, номеру автомобиля или номеру дела" /></label><div class="case-status-filters"><button v-for="filter in [['ALL', 'Все'], ['CREATED', 'Создан'], ['WAITING_PARTS', 'Ждём детали'], ['PARTS_RECEIVED', 'Детали поступили'], ['SCHEDULED', 'Запись на ремонт'], ['IN_REPAIR', 'Ремонт'], ['READY', 'Готов к выдаче'], ['DELIVERED', 'Выдан'], ['CLOSED', 'Закрыт']]" :key="filter[0]" type="button" class="filter" :class="{ 'is-active': caseStatusFilter === filter[0] }" @click="caseStatusFilter = filter[0]">{{ filter[1] }}</button></div><button class="button button-primary" type="button" @click="openRepairMenu">＋ Создать страховой случай</button></div>
