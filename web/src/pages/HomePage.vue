...
const resolvedPartsChoiceVisible = ref(false)
const openCasePartStatusMenuId = ref(null)
const casePartEditDrafts = ref({})
...
const partFormReturnToCaseDetail = ref(false)
...
watch(partFormVisible, (visible, previous) => {
  if (!visible && previous && casePartContext.value && partFormReturnToCaseDetail.value) {
    caseDetailTab.value = 'parts'
    caseDetailVisible.value = true
  }
  if (!visible && previous) partFormReturnToCaseDetail.value = false
})
...
function emptyPartForm() {
  return { name: '', article: '', catalogNumber: '', manufacturer: '', quantity: 1, supplierId: '', orderedAt: '', expectedDate: '', comment: '', received: false, declined: false, sortOrder: 0 }
}
...
async function toggleCasePartReceived(item, part) {
  try {
    ...
    openCasePartStatusMenuId.value = null
    showToast(updated.received ? `Деталь «${updated.name}» получена` : `Поступление детали «${updated.name}» отменено`)
  } catch (error) { showToast(error.message) }
}

async function toggleCasePartDeclined(item, part) {
  try {
    const updated = await requestJson(`/api/v1/cars/${item.carId}/repair-cases/${item.id}/parts/${part.id}/decline?declined=${!part.declined}`, { method: 'PATCH' })
    expandedCaseParts.value = expandedCaseParts.value.map((value) => value.id === updated.id ? updated : value)
    await loadRepairRegistry()
    openCasePartStatusMenuId.value = null
    showToast(updated.declined ? `Отказ от детали «${updated.name}» сохранён` : `Деталь «${updated.name}» снова ожидается`)
  } catch (error) { showToast(error.message) }
}

function setCasePartDraft(part, field, value) {
  casePartEditDrafts.value = { ...casePartEditDrafts.value, [part.id]: { ...(casePartEditDrafts.value[part.id] || {}), [field]: value } }
}

async function saveCasePartEdit(item, part) {
  const draft = casePartEditDrafts.value[part.id]
  if (!draft) return
  const nextPart = { ...part, ...draft }
  try {
    const updated = await requestJson(`/api/v1/cars/${item.carId}/repair-cases/${item.id}/parts/${part.id}`, {
      method: 'PUT',
      body: JSON.stringify({ name: nextPart.name, article: nextPart.article || '', catalogNumber: nextPart.catalogNumber || '', manufacturer: nextPart.manufacturer || '', quantity: Number(nextPart.quantity || 1), supplierId: nextPart.supplierId || null, orderedAt: nextPart.orderedAt || null, expectedDate: nextPart.expectedDate || null, comment: nextPart.comment || '', received: !!nextPart.received, declined: !!nextPart.declined, sortOrder: nextPart.sortOrder || 0 }),
    })
    expandedCaseParts.value = expandedCaseParts.value.map((value) => value.id === updated.id ? updated : value)
    await loadRepairRegistry()
    const nextDrafts = { ...casePartEditDrafts.value }
    delete nextDrafts[part.id]
    casePartEditDrafts.value = nextDrafts
    showToast('Поставщик и дата доставки сохранены')
  } catch (error) {
    showToast(error.message)
  }
}
...
function openDirectDeliveryAction() {
  resolvedPartsChoiceVisible.value = false
  caseActionModal.value = 'DELIVER'
  caseActionForm.value = { contractorId: '', appointmentDate: '', appointmentTime: '', receivedBy: 'Система', comment: '' }
}
...
const caseAllPartsDeclined = computed(() => caseParts.value.length > 0 && caseParts.value.every((part) => part.declined))
...
function openPartForm(car, caseItem = caseDetailVisible.value ? selectedRegistryCase.value : null, returnToCaseDetail = caseDetailVisible.value) {
  loadSuppliers()
  partFormReturnToCaseDetail.value = Boolean(caseItem && returnToCaseDetail)
...
async function savePart() {
  ...
  try {
    ...
    await loadCars()
    if (casePartContext.value) {
      caseParts.value = await requestJson(`/api/v1/cars/${selectedCar.value.id}/repair-cases/${casePartContext.value.id}/parts`)
      if (expandedCaseId.value === casePartContext.value.id) expandedCaseParts.value = caseParts.value
      await loadRepairRegistry()
      selectedRegistryCase.value = repairRegistry.value.find((item) => item.id === casePartContext.value.id) || selectedRegistryCase.value
    }
...
function closeCasePartStatusMenu(event) {
  if (!event.target.closest?.('.case-part-status-actions')) openCasePartStatusMenuId.value = null
}
...
onMounted(() => {
  ...
  document.addEventListener('click', closeCasePartStatusMenu)
...
onUnmounted(() => {
  ...
  document.removeEventListener('click', closeCasePartStatusMenu)
  ...
  <template v-else>
    ...
    <main class="page-shell">
      ...
      <section v-if="activeSection === 'cases'" class="workspace cases-workspace">
        ...
        <div class="cars-list"><template v-for="item in visibleRepairCases" :key="item.id"><article class="car-row case-row" :class="{ 'is-selected': expandedCaseId === item.id }" style="grid-template-columns: .55fr 1.35fr 1.2fr 1.2fr 1.05fr 1.15fr 1fr .9fr;"><div class="cell"><strong>{{ item.vehicleMake }} {{ item.vehicleModel }}</strong><small>VIN {{ item.vin }}</small><small>{{ item.registrationNumber }} · {{ item.ownerName }} · {{ item.ownerPhone }}</small></div><div class="cell"><strong>№{{ item.caseNumber }}</strong></div><div class="cell"><span class="insurance-pill">{{ insurers.find((insurer) => insurer.id === item.insurerId)?.name || 'Страховая не указана' }}</span></div><div class="cell inline-contractor-cell"><select v-if="editingContractorCaseId === item.id" class="inline-contractor-select" :value="String(item.contractorId || '')" :disabled="savingContractorCaseId === item.id" @click.stop @change.stop="saveInlineCaseContractor(item, $event)"><option value="" disabled>Выберите исполнителя</option><option v-for="contractor in contractors" :key="contractor.id" :value="String(contractor.id)">{{ contractor.shortName || contractor.fullName }}</option></select><button v-else type="button" class="inline-contractor-button" @click.stop="editingContractorCaseId = item.id; loadContractors()">{{ contractors.find((contractor) => contractor.id === item.contractorId)?.shortName || 'Не назначен' }}</button></div><div class="cell inline-appointment-date-cell"><input v-if="item.appointmentDate" type="date" class="inline-appointment-date-input" :value="item.appointmentDate" :disabled="item.status === 'DELIVERED' || item.status === 'CLOSED' || savingAppointmentDateCaseId === item.id" @click.stop @change.stop="saveInlineAppointmentDate(item, $event)" /><span v-else class="inline-appointment-date-hint" data-tooltip="Чтобы выставить дату, переведите статус случая «Запись на ремонт»" aria-label="Чтобы выставить дату, переведите статус случая «Запись на ремонт»">—</span></div><div class="cell case-parts-progress-cell"><div class="parts-progress-ring" :style="{ '--parts-progress': item.partsTotal ? ((item.partsReceived + (item.partsDeclined || 0)) / item.partsTotal) * 100 + '%' : '0%' }"><span>{{ (item.partsReceived || 0) + (item.partsDeclined || 0) }}/{{ item.partsTotal || 0 }}</span></div><div><strong>{{ (item.partsReceived || 0) + (item.partsDeclined || 0) }} из {{ item.partsTotal || 0 }}</strong><small>{{ item.partsTotal ? (item.partsReceived + (item.partsDeclined || 0) === item.partsTotal ? (item.partsDeclined ? 'Детали закрыты' : 'Поступление по графику') : 'Ожидание деталей') : 'Детали не добавлены' }}</small></div></div><div class="cell"><span class="case-status">{{ statusLabel(item.status) }}</span></div><div class="cell muted-cell"><button type="button" class="case-row-details-button" @click.stop="toggleCaseRow(item)">{{ expandedCaseId === item.id ? 'Скрыть детали' : 'Открыть' }}</button></div></article><section v-if="expandedCaseId === item.id" class="case-row-details"><div class="case-row-details-head"><strong>Детали страхового случая №{{ item.caseNumber }}</strong><div class="case-row-details-actions"><span>{{ expandedCaseParts.length }} деталей · {{ expandedCaseParts.filter((part) => part.received).length }} получено</span><button type="button" class="case-row-add-part-compact" @click.stop="openPartForm({ id: item.carId, number: item.accountingNumber, vehicle: item.vehicleMake + ' ' + item.vehicleModel }, item, false)">＋ Добавить деталь</button></div></div><div v-if="expandedCaseParts.length" class="case-parts-table"><div class="case-parts-table-head"><span>Деталь</span><span>Артикул</span><span>Поставщик</span><span>Плановая поставка</span><span>Статус</span><span>Поступила</span></div><div v-for="part in expandedCaseParts" :key="part.id" class="case-parts-table-row"><span><strong>{{ part.name }}</strong><small v-if="part.comment">{{ part.comment }}</small></span><span>{{ part.article || '—' }}</span><span class="case-part-editable-cell"><select class="case-part-inline-select" :value="casePartEditDrafts[part.id]?.supplierId ?? part.supplierId ?? ''" @click.stop @change.stop="setCasePartDraft(part, 'supplierId', $event.target.value ? Number($event.target.value) : null)"><option value="">—</option><option v-for="supplier in suppliers" :key="supplier.id" :value="supplier.id">{{ supplier.name }}</option></select></span><span class="case-part-editable-cell"><input class="case-part-inline-date" type="date" :value="casePartEditDrafts[part.id]?.expectedDate ?? part.expectedDate ?? ''" @click.stop @change.stop="setCasePartDraft(part, 'expectedDate', $event.target.value || null)" /></span><span class="case-part-status-cell"><div class="case-part-status-actions"><button type="button" class="part-status-trigger" :class="part.declined ? 'is-declined' : (part.received ? 'is-received' : 'is-waiting')" @click.stop="openCasePartStatusMenuId = openCasePartStatusMenuId === part.id ? null : part.id"><b>{{ part.declined ? 'Отказались' : (part.received ? 'Поступила' : 'Ожидается') }}</b><span>⌄</span></button><div v-if="openCasePartStatusMenuId === part.id" class="part-status-menu"><button type="button" :class="{ 'is-active': part.received }" @click.stop="toggleCasePartReceived(item, part)">Поступила</button><button type="button" :class="{ 'is-active': part.declined }" @click.stop="toggleCasePartDeclined(item, part)">Отказ</button></div><button v-if="casePartEditDrafts[part.id]" type="button" class="case-part-save-button" @click.stop="saveCasePartEdit(item, part)">Сохранить</button></div></span><span class="case-received-date">{{ part.received ? part.receivedAt : '—' }}</span></div></div><div v-else class="case-row-empty-parts"><p class="empty-state">У этого страхового случая деталей пока нет.</p><button type="button" class="button button-primary case-row-add-part-button" @click.stop="openPartForm({ id: item.carId, number: item.accountingNumber, vehicle: item.vehicleMake + ' ' + item.vehicleModel }, item, false)">＋ Добавить деталь</button></div></section></template><p v-if="!visibleRepairCases.length" class="empty-state">Страховых случаев пока нет. Создайте первый через «＋ Ремонт».</p></div>
...
.case-row-details { margin: -1px 0 10px; padding: 16px 20px 20px; border: 1px solid var(--line); border-top: 0; background: #fbfdfc; overflow: visible; }
.case-row-empty-parts { display: flex; align-items: center; justify-content: center; gap: 16px; padding: 28px 20px; }
.case-row-empty-parts .empty-state { padding: 0; margin: 0; }
.case-row-add-part-button { min-height: 38px; white-space: nowrap; }
.case-row-details-head { display: flex; justify-content: space-between; gap: 16px; margin-bottom: 12px; color: var(--muted); }
.case-row-details-actions { display: flex; align-items: center; justify-content: flex-end; gap: 12px; }.case-row-add-part-compact { min-height: 32px; padding: 0 11px; border: 1px solid var(--accent); border-radius: 8px; color: var(--brand); background: var(--accent-soft); font-size: 11px; font-weight: 800; white-space: nowrap; cursor: pointer; }.case-row-add-part-compact:hover { background: var(--accent); }
.case-parts-table { overflow: visible; border: 1px solid var(--line); border-radius: 10px; }
...
.case-parts-table-row { position: relative; border-top: 1px solid var(--line); }
.case-parts-table-row span:first-child { display: grid; gap: 3px; }
.case-parts-table-row small { color: var(--muted); }
.case-part-editable-cell { min-width: 0; }.case-part-inline-select, .case-part-inline-date { width: 100%; min-height: 32px; padding: 0 8px; border: 1px solid transparent; border-radius: 8px; color: var(--ink); background: transparent; font: inherit; }.case-part-inline-select:hover, .case-part-inline-select:focus, .case-part-inline-date:hover, .case-part-inline-date:focus { border-color: var(--line); background: white; outline: none; }.case-part-inline-date { min-width: 126px; }
.case-part-save-button { min-height: 30px; padding: 0 10px; border: 1px solid var(--accent); border-radius: 8px; color: var(--brand); background: var(--accent-soft); font-size: 11px; font-weight: 800; cursor: pointer; }.case-part-save-button:hover { background: var(--accent); }
...
.case-part-status-cell { display: grid; justify-items: start; gap: 4px; }.case-part-status-cell .part-receipt-button, .case-part-status-cell .part-decline-button { padding: 4px 7px; min-height: 26px; font-size: 10px; }
.part-status-trigger { display: inline-flex; align-items: center; gap: 8px; min-width: 118px; justify-content: space-between; padding: 7px 10px; border: 1px solid var(--line); border-radius: 9px; color: var(--ink); background: white; cursor: pointer; }.part-status-trigger span { color: var(--muted); font-size: 12px; }.part-status-trigger.is-waiting { border-color: #f1d88c; background: #fff1c5; color: #996900; }.part-status-trigger.is-received { border-color: #9adbc3; background: #e2f7ee; color: #16815c; }.part-status-trigger.is-declined { border-color: #e7c794; background: #fff3dc; color: #9a6911; }.part-status-menu { position: absolute; z-index: 5; display: grid; gap: 3px; min-width: 118px; margin-top: 4px; padding: 4px; border: 1px solid var(--line); border-radius: 9px; background: white; box-shadow: 0 8px 20px rgb(8 43 37 / 14%); }.part-status-menu button { padding: 7px 9px; border: 0; border-radius: 6px; color: var(--muted); background: transparent; text-align: left; cursor: pointer; }.part-status-menu button:hover, .part-status-menu button.is-active { color: var(--brand); background: var(--accent-soft); }
.part-decline-button { border: 1px solid var(--line); border-radius: 8px; color: var(--muted); background: var(--soft); cursor: pointer; }.part-decline-button:hover { color: var(--brand); border-color: var(--accent); background: var(--accent-soft); }.part-declined { color: var(--amber); }
.case-part-status-actions { position: relative; z-index: 3; display: flex; align-items: center; gap: 8px; flex-wrap: wrap; white-space: normal; }.case-part-status-actions .part-receipt-button { margin: 0; }.case-received-date { color: var(--muted); font-size: 12px; }
...
.resolved-parts-choice-overlay { z-index: 30; }
.resolved-parts-choice-actions { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; margin-top: 22px; }
.resolved-parts-choice-actions .button { min-height: 46px; }
...
.inline-appointment-date-hint { position: relative; display: inline-flex; width: 142px; min-height: 34px; align-items: center; padding: 0 7px; border: 1px dashed var(--line); border-radius: 8px; color: var(--muted); background: var(--soft); font-size: 11px; cursor: help; }
.inline-appointment-date-hint::after { content: attr(data-tooltip); position: absolute; left: 50%; bottom: calc(100% + 7px); z-index: 5; width: max-content; max-width: 300px; padding: 8px 10px; transform: translateX(-50%); border-radius: 7px; color: #fff; background: var(--brand); box-shadow: 0 6px 18px #133a3340; font-size: 12px; line-height: 1.35; opacity: 0; pointer-events: none; transition: opacity .08s ease; }
.inline-appointment-date-hint:hover::after { opacity: 1; }
