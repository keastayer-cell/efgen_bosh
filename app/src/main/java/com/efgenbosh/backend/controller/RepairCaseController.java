...
public class RepairCaseController {
    ...
    public RepairCaseResponse create(@PathVariable Long carId, @Valid @RequestBody RepairCaseRequest request, Authentication authentication) {
        RepairCase item = new RepairCase(); item.setCar(ensureCar(carId)); apply(item, request); item.setStatus("CREATED"); item.setCreatedBy(userId(authentication)); item.setUpdatedBy(userId(authentication)); return RepairCaseResponse.from(cases.save(item));
    }
    @PutMapping("/{caseId}")
    public RepairCaseResponse update(@PathVariable Long carId, @PathVariable Long caseId, @Valid @RequestBody RepairCaseRequest request, Authentication authentication) {
        RepairCase item = cases.findById(caseId).filter(value -> value.getCar().getId().equals(carId)).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Страховой случай не найден.")); apply(item, request); item.setUpdatedBy(userId(authentication)); return RepairCaseResponse.from(cases.save(item));
    }
    ...
    private void apply(RepairCase item, RepairCaseRequest request) { item.setCaseNumber(request.caseNumber().trim()); item.setInsuredPerson(value(request.insuredPerson())); item.setClaimNumber(value(request.claimNumber())); item.setInsurerId(request.insurerId()); item.setContractorId(request.contractorId()); item.setShiftId(request.shiftId()); item.setAcceptedAt(request.acceptedAt()); item.touch(); }
