...
public class RepairCasePartController {
    ...
    private void ensureEditable(com.efgenbosh.backend.domain.RepairCase item){if(RepairCaseStatus.DELIVERED.code().equals(item.getStatus())) throw new ResponseStatusException(HttpStatus.CONFLICT,"Выданная машина доступна только для просмотра.");}
