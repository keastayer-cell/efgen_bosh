...
public class RepairCasePartController {
    ...
    @PostMapping @ResponseStatus(HttpStatus.CREATED) public PartResponse create(@PathVariable Long carId,@PathVariable Long caseId,@Valid @RequestBody PartRequest request,org.springframework.security.core.Authentication authentication){var repairCase=ensureCase(carId,caseId);ensureEditable(repairCase);Part part=new Part();part.setCar(repairCase.getCar());part.setRepairCase(repairCase);apply(part,request);var saved=parts.save(part);record(repairCase,"Добавлена деталь: " + saved.getName(), userId(authentication));return PartResponse.from(saved,LocalDate.now());}
    @PutMapping("/{partId}") public PartResponse update(@PathVariable Long carId,@PathVariable Long caseId,@PathVariable Long partId,@Valid @RequestBody PartRequest request,org.springframework.security.core.Authentication authentication){var repairCase=ensureCase(carId,caseId);ensureEditable(repairCase);Part part=parts.findById(partId).filter(value->value.getRepairCase()!=null&&value.getRepairCase().getId().equals(caseId)).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"Запчасть не найдена."));apply(part,request);var saved=parts.save(part);if(saved.isReceived() && parts.findAllByRepairCase_IdOrderBySortOrderAscIdAsc(caseId).stream().allMatch(Part::isReceived)){String previous=repairCase.getStatus();repairCase.setStatus("PARTS_RECEIVED");cases.save(repairCase);record(repairCase,"Все детали получены: " + previous + " → Детали поступили", userId(authentication));}else record(repairCase,"Изменена деталь: " + saved.getName(), userId(authentication));return PartResponse.from(saved,LocalDate.now());}
    ...
    private void ensureEditable(com.efgenbosh.backend.domain.RepairCase item){if("CLOSED".equals(item.getStatus())) throw new ResponseStatusException(HttpStatus.CONFLICT,"Закрытый страховой случай доступен только для просмотра.");}
